...
import net.peacefulcraft.xcom.xcom_client_bukkit.XCOMProxy;
...
public abstract class Configuration {
	...
	protected void createDefaultConfiguration(Boolean empty) {
		try {
			this.configFile = new File(XCOMProxy._this().getDataFolder(), this.configName);
			if (!configFile.exists()) {
				...
				XCOMProxy._this().logNotice("Found existing file at " + this.configName + " - not creating a new one");
			}
		} catch (IOException e) {
			XCOMProxy._this().logSevere("Error initializing config file " + this.configName);
			e.printStackTrace();
		}
	}

	protected void loadConfiguration() {
		config = YamlConfiguration.loadConfiguration(new File(XCOMProxy._this().getDataFolder(), this.configName));
	}
	...
	public void saveConfiguration() {
		try {
			...
			e.printStackTrace();
			XCOMProxy._this().logSevere("Unable to save configuration file.");
		  }
	}
}